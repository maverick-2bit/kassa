...
export function KundenPage() {
  ...
  const [bearbeitete,      setBearbeitete]      = useState<Kunde | null>(null)
  const [verlaufKunde,     setVerlaufKunde]     = useState<Kunde | null>(null)
  const [lieferscheineKunde,  setLiferscheineKunde]  = useState<Kunde | null>(null)
  const [offenePostenKunde,   setOffenePostenKunde]   = useState<Kunde | null>(null)
  ...
  const aktualisiereMutation = useMutation({
    mutationFn: ({ id, input }: { id: string; input: KundeUpdate }) => kundeApi.update(id, input),
    onSuccess:  () => { qc.invalidateQueries({ queryKey: ['kunden'] }); setBearbeitete(null) },
    onError:    (err) => setFehler(err instanceof Error ? err.message : String(err)),
  })

  const deactivateMutation = useMutation({
    mutationFn: kundeApi.deactivate,
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['kunden'] }),
    onError:    (err) => setFehler(err instanceof Error ? err.message : String(err)),
  })

  const reactivateMutation = useMutation({
    mutationFn: kundeApi.reactivate,
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['kunden'] }),
    onError:    (err) => setFehler(err instanceof Error ? err.message : String(err)),
  })
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:py-8 space-y-6">
              ...
              <tbody className="divide-y divide-gray-100">
                {kunden.map(k => (
                  <tr key={k.id} className={`hover:bg-gray-50 ${!k.aktiv ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-2 text-gray-500 font-mono text-xs">{k.uid ?? '—'}</td>
                    <td className="px-4 py-2">
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button
                          type="button"
                          onClick={() => setLiferscheineKunde(k)}
                        ...
                        <button
                          type="button"
                          onClick={() => setOffenePostenKunde(k)}
                        ...
                        <button
                          type="button"
                          onClick={() => { setFehler(null); setBearbeitete(k) }}
      ...
      {/* Bearbeiten */}
      {bearbeitete && (
        <Modal open={true} onClose={() => setBearbeitete(null)} title={`Kunde #${bearbeitete.nummer}`}>
          <KundeFormular
            initial={bearbeitete}
            onSubmit={(input) => aktualisiereMutation.mutate({ id: bearbeitete.id, input: input as KundeUpdate })}
            onAbbrechen={() => setBearbeitete(null)}
      ...
      {verlaufKunde && (
        <Modal
          open={true}
          onClose={() => setVerlaufKunde(null)}
          title={`Rechnungshistorie – ${verlaufKunde.bezeichnung}`}
        >
          <VerlaufModal kundeId={verlaufKunde.id} />
        </Modal>
      )}
      ...
      {lieferscheineKunde && (
        <Modal
          open={true}
          onClose={() => setLiferscheineKunde(null)}
          title={`Lieferscheine – ${lieferscheineKunde.bezeichnung}`}
          size="xl"
        >
          <LiferscheineModal
            kunde={lieferscheineKunde}
            onClose={() => setLiferscheineKunde(null)}
          />
        </Modal>
      )}
      ...
      {offenePostenKunde && (
        <Modal
          open={true}
          onClose={() => setOffenePostenKunde(null)}
          title={`Offene Posten – ${offenePostenKunde.bezeichnung}`}
          size="xl"
        >
          <OffenePostenKundeModal
            kunde={offenePostenKunde}
            onClose={() => setOffenePostenKunde(null)}
          />
        </Modal>
      )}
...
function LiferscheineModal({ kunde, onClose }: { kunde: Kunde; onClose: () => void }) {
  const qc = useQueryClient()
  const auth = getAuth()
  ...
  const alleToggle = () => {
    const offene = lieferscheine.filter(ls => ls.status === 'offen').map(ls => ls.id)
    if (ausgewaehlt.size === offene.length && offene.every(id => ausgewaehlt.has(id))) {
      setAusgewaehlt(new Set())
    } else {
      setAusgewaehlt(new Set(offene))
    }
  }
              ...
              <tbody className="divide-y divide-gray-100">
                {lieferscheine.map(ls => {
                  ...
                  return (
                    <tr
                      key={ls.id}
                      className={`hover:bg-gray-50 transition ${isChecked ? 'bg-blue-50' : ''}`}
                    >
...
function VerlaufModal({ kundeId }: { kundeId: string }) {
  const { data: belege = [], isLoading } = useQuery({
    queryKey: ['kunden-belege', kundeId],
    queryFn:  () => kundeApi.belege(kundeId),
  })
  ...
  if (isLoading) {
    return <p className="text-sm text-gray-400 text-center py-8">Lade…</p>
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-lg bg-brand-50 border border-brand-200 px-4 py-3">
          ...
          <p className="text-xl font-bold text-brand-800 mt-0.5">{formatPreis(gesamtUmsatz)}</p>
        </div>
      </div>
...
function OffenePostenKundeModal({ kunde, onClose }: { kunde: Kunde; onClose: () => void }) {
  const qc = useQueryClient()
    ...
    <div className="space-y-4">
                ...
                <tr key={op.id} className={`border-b border-gray-100 last:border-0 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}`}>
                  ...
                  <td className="px-3 py-2.5 text-right">
                    {op.status !== 'bezahlt' && (
                      <button
                        type="button"
                        onClick={() => { setZahlungModal(op); setZahlungEuro((op.restCent / 100).toFixed(2).replace('.', ',')); setFehler(null) }}
      ...
      {openPosten && (
        <Modal
          ...
          title={`Zahlung – OP-${String(openPosten.nummer).padStart(4, '0')}`}
        >
          <div className="space-y-4">
            <div className="rounded-lg bg-gray-50 border border-gray-200 p-4 space-y-1.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-mono">{formatPreis(openPosten.betragCent)}</span>
              </div>
              {openPosten.bezahltCent > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Bereits bezahlt</span>
                  <span className="font-mono text-green-700">{formatPreis(openPosten.bezahltCent)}</span>
                </div>
              )}
              <div className="flex justify-between border-t border-gray-200 pt-1.5 font-semibold">
                <span>Restbetrag</span>
                <span className="font-mono text-red-700">{formatPreis(openPosten.restCent)}</span>
              </div>
            </div>
              ...
              <Input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter') {
                    ...
                    if (!isNaN(cents) && cents > 0) zahlungMutation.mutate({ id: openPosten.id, zahlungCent: cents })
                  }
                }}
                className="mt-1"
              />
            ...
            <div className="flex gap-2 pt-1">
              <Button variant="secondary" onClick={() => { setZahlungModal(null); setZahlungEuro(''); setFehler(null) }} className="flex-1">
                Abbrechen
              </Button>
              <Button
                onClick={() => {
                  ...
                  zahlungMutation.mutate({ id: openPosten.id, zahlungCent: cents })
                }}
